...
isMother(X) :- setof(Z, W^(female(Z), parent(Z, W)), Y), !, member(X, Y).
isFather(X) :- setof(Z, W^(male(Z), parent(Z, W)), Y), !, member(X, Y).

% 3. Define a predicate `grandparent/2`.
grandparent(X, Y) :- parent(X, Z), parent(Z, Y).

% 4. Define a predicate `sibling/2`. Siblings share at least one parent.
sibling(X, Y) :- setof(Z, (parent(Z, X), parent(Z, Y), X \= Y), _).
...
aunt(X, Y) :- married(X, Z), sibling(Z, W), parent(W, Y), female(X).
